<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Añada seguridad adicional, activando la Autenticación de dos factores.') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Finalizando la activación de Autenticación de dos factores.') }}
                @else
                    {{ __('Tienes activada la Autenticación de dos factores.') }}
                @endif
            @else
                {{ __('No tiene activada la Autenticación de dos factores.') }}
            @endif
        </h3>

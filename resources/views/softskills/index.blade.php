    ...
    <div class="overflow-hidden bg-white shadow-lg sm:rounded-lg sm:mx-12 md:mx-52 mt-10 ">
        <div class="px-4 py-5 sm:px-6">
            ...
            <div class="flex">
                ...
                <div class="mt-5">
                    <button onclick="location.href='{{ route('softskills.create') }}'" type="button"
            ...
            <dl>
                ...
                @foreach ($softskills as $softskill)
                    <div class="{{ $color }} px-4 py-5 grid grid-cols-12">
                        <div class="text-sm  font-medium text-gray-500">{{ $softskill->id }}</div>
                        <div class="mt-1 text-sm col-span-8 text-gray-900 sm:mt-0">{{ $softskill->name }}</div>
                        <div class="mt-1 sm:mt-0">
                            <form action="/softskills/{{ $softskill->id }}" method="POST">
                                @csrf
                                {{ method_field('DELETE') }}
                                <button type="submit"

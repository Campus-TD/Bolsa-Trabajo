    ...
    <div class="overflow-hidden bg-white shadow-lg sm:rounded-lg sm:mx-12 md:mx-52 mt-10 ">
        <div class="px-4 py-5 sm:px-6">
            ...
            <div class="flex">
                ...
                <div class="mt-5">
                    <button onclick="location.href='{{ route('hardskills.create') }}'" type="button"
                        class="inline-flex rounded bg-violet-600 py-2 px-3 text-sm font-medium text-white shadow-sm transition-all duration-500 hover:bg-violet-300 focus:outline-none">Nueva
                        Habilidad
                    </button>
                </div>
            ...
            <dl>
                ...
                @foreach ($hardskills as $hardskill)
                    <div class="{{ $color }} px-4 py-5 grid grid-cols-12">
                        <div class="text-sm  font-medium text-gray-500">{{ $hardskill->id }}</div>
                        <div class="mt-1 text-sm col-span-8 text-gray-900 sm:mt-0">{{ $hardskill->name }}</div>
                        <div class="mt-1 sm:mt-0">
                            <form action="/hardskills/{{ $hardskill->id }}" method="POST">
                                @csrf
                                {{ method_field('DELETE') }}
                                <button type="submit"
